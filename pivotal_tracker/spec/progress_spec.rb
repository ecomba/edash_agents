...
describe Edash::Agents::Pivotal::Tracker::Progress do
  context 'reading xml' do
    context 'status: started' do
        subject { Edash::Agents::Pivotal::Tracker::Progress.new(xml('started')) }
        its(:state) { should == 'started' }
    end

    context 'status: unstarted' do 
      subject { Edash::Agents::Pivotal::Tracker::Progress.new(xml('unstarted')) }
      its(:state) { should == 'unstarted' }
    end

    context 'status: finshed' do 
      subject { Edash::Agents::Pivotal::Tracker::Progress.new(xml('finished')) }
